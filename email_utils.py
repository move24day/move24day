...
def send_quote_email(recipient_email, subject, body, pdf_bytes, pdf_filename="견적서.pdf"):
    ...
    try:
        ...
        print(f"DEBUG: Loaded email credentials for {sender_email}") # 디버깅
    ...
    print(f"DEBUG: Email object created for {recipient_email}") # 디버깅
    ...
    if pdf_bytes:
        try:
            ...
            message.attach(part)
            print(f"DEBUG: PDF attached with filename: {pdf_filename}") # 디버깅
        except Exception as e:
    ...
    try:
        ...
        print(f"DEBUG: Connecting to SMTP: {smtp_server}:{smtp_port}") # 디버깅
        # 포트에 따라 SMTP 또는 SMTP_SSL 사용 및 starttls 호출 결정
        if smtp_port == 465: # SSL 포트
             server = smtplib.SMTP_SSL(smtp_server, smtp_port, context=context)
             print("DEBUG: Using SMTP_SSL.") # 디버깅
        elif smtp_port == 587: # TLS 포트
             server = smtplib.SMTP(smtp_server, smtp_port)
             print("DEBUG: Using SMTP, attempting STARTTLS...") # 디버깅
             server.ehlo() # Can be helpful before starttls
             server.starttls(context=context) # TLS 보안 연결 시작
             server.ehlo() # Starttls 후 다시 ehlo
             print("DEBUG: STARTTLS successful.") # 디버깅
        else: # 기타 포트
             server = smtplib.SMTP(smtp_server, smtp_port)
             print(f"DEBUG: Using SMTP on port {smtp_port}. Consider security.") # 디버깅
             # 필요시 server.starttls() 또는 다른 보안 설정 추가

        print(f"DEBUG: Logging in as {sender_email}") # 디버깅
        server.login(sender_email, sender_password)
        print("DEBUG: Login successful.") # 디버깅

        print(f"DEBUG: Sending email to {recipient_email}...") # 디버깅
        server.send_message(message) # 메시지 발송
        print(f"DEBUG: Email sent successfully to {recipient_email}") # 디버깅
        return True
        ...
        return False
         ...
         # 네트워크 관련 오류 (예: [Errno 111] Connection refused)
        ...
        if server:
            try:
                 server.quit()
                 print("DEBUG: SMTP connection closed.") # 디버깅
            except Exception as e:
