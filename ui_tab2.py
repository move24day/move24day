# ui_tab2.py (Updated info message)
    ...
    with st.container(border=True):
        ...
        for section, item_list in item_category_to_display.items():
            ...
            with st.expander(expander_label, expanded=expanded_default):
                # Basket section specific info (Updated)
                if section == basket_section_name_check:
                    ...
                    if selected_truck_tab2 and selected_truck_tab2 in data.default_basket_quantities:
                        # Use the updated default_basket_quantities from data.py
                        defaults = data.default_basket_quantities[selected_truck_tab2]
                        basket_qty = defaults.get('바구니', 0)
                        box_qty = defaults.get('중박스', 0) # Get new default box quantity
                        clothes_qty = defaults.get('옷바구니', 0) # Get new clothes basket quantity
                        book_qty = defaults.get('책바구니', 0)
                        # Update info message
                        st.info(f"💡 **{selected_truck_tab2}** 추천 기본값: 바구니 {basket_qty}개, 중박스 {box_qty}개, 옷바구니 {clothes_qty}개, 책바구니 {book_qty}개 (현재 값이며, 직접 수정 가능합니다)")
                    else:
                        st.info("💡 비용 탭에서 차량 선택 시 추천 기본 바구니 개수가 여기에 표시됩니다.")

                # Display number inputs (Remains the same)
                ...
                if isinstance(item_list_move, list):
                    for item_move in item_list_move:
                        ...
                        if widget_key_move in st.session_state:
                            ...
                            if qty > 0: unit_move = "칸" if item_move == "장롱" else "개"; move_selection_display[item_move] = (qty, unit_move)
                        processed_items_summary_move.add(item_move)
        if move_selection_display:
            st.markdown("**선택 품목 목록:**"); cols_disp_m = st.columns(2)
            ...
            total_vol = st.session_state.get('total_volume', 0.0); total_wt = st.session_state.get('total_weight', 0.0)
            ...
            if recommended_vehicle_display and "초과" not in recommended_vehicle_display:
                 ...
                 st.success(rec_text)
                 if final_vehicle_tab2_display and final_vehicle_tab2_display != recommended_vehicle_display: st.warning(f"⚠️ 현재 비용계산 탭에서 **{final_vehicle_tab2_display}** 차량이 최종 선택되어 있습니다.")
                 ...
                 if total_vol > 0 or total_wt > 0: st.warning("⚠️ 추천 차량: 자동 추천 불가. 비용계산 탭에서 차량을 수동 선택해주세요.")
                 else: st.info("ℹ️ 이사할 품목이 없습니다. 품목을 선택해주세요.")
                 if final_vehicle_tab2_display: st.info(f"ℹ️ 현재 비용계산 탭에서 **{final_vehicle_tab2_display}** 차량이 수동 선택되어 있습니다.")
        else: st.info("ℹ️ 선택된 이사 품목이 없습니다. 위에서 품목을 선택해주세요.")

# --- End of render_tab2 function ---
