...
def generate_pdf(state_data, calculated_cost_items, total_cost, personnel_info):
    ...
    try:
        ...
        moving_date_str = str(state_data.get('moving_date', '-'))
        ...
        info_pairs = [
            ("고 객 명:", customer_name), ("연 락 처:", customer_phone),
            ("이 사 일:", moving_date_str), ("견 적 일:", kst_date_str),
            ("출 발 지:", from_location), ("도 착 지:", to_location)
        ]
        if is_storage:
        ...
        for label, value in info_pairs:
             ...
             value_para.drawOn(c, value_x, para_y_pos); current_y -= row_height
              ...
              if str(item[0]) == "기본 운임":
                 ...
                 if date_surcharge_index != -1 and date_surcharge_amount > 0:
                     try:
                         ...
                         item[2] = f"{selected_vehicle_remark} (이사 집중일 운영 요금 적용)" # 비고 설정
        ...
        if special_notes:
            ...
            else: current_y -= line_height # 섹션 제목 위한 공간
            c.setFont('NanumGothicBold', 11); c.drawString(margin_x, current_y, "[ 고객요구사항 ]"); current_y -= line_height * 1.2 # 제목 아래 여백
            ...
            for note_part in notes_parts:
                ...
                if current_y - part_height < margin_y: c.showPage(); page_number += 1; draw_page_template(c, page_number); current_y = height - margin_y - 1*cm; c.setFont('NanumGothic', 11) # 새 페이지 Y 위치 초기화 및 폰트 리셋
