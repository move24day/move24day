# pdf_generator.py (utils.get_item_qty 호출 확인)

import pandas as pd
import io
import streamlit as st
import traceback
import utils # utils.py 필요 (get_item_qty 사용)
...
# --- 상수 및 폰트 경로 (기존과 동일) ---
COMPANY_ADDRESS = "서울 은평구 가좌로10길 33-1"; COMPANY_PHONE_1 = "010-5047-1111"; COMPANY_PHONE_2 = "1577-3101"; COMPANY_EMAIL = "move24day@gmail.com"; NANUM_GOTHIC_FONT_PATH = "NanumGothic.ttf"

# --- PDF 생성 함수 (generate_pdf) ---
# 이 함수는 이전 버전 (기본운임 비고, 고객요구사항 줄바꿈 수정된 버전) 유지
def generate_pdf(state_data, calculated_cost_items, total_cost, personnel_info):
    # ... (이전 답변에서 제공된 generate_pdf 함수 전체 내용 - 변경 없음) ...
    buffer = io.BytesIO()
    try:
        # 폰트 등록
        if not os.path.exists(NANUM_GOTHIC_FONT_PATH): st.error(f"..."); return None
        try: pdfmetrics.registerFont(TTFont('NanumGothic', NANUM_GOTHIC_FONT_PATH)); pdfmetrics.registerFont(TTFont('NanumGothicBold', NANUM_GOTHIC_FONT_PATH))
        except Exception as font_e: st.error(f"..."); return None

        c = canvas.Canvas(buffer, pagesize=A4); width, height = A4; margin_x = 1.5*cm; margin_y = 1.5*cm; line_height = 0.6*cm; right_margin_x = width - margin_x; page_number = 1

        def draw_page_template(canvas_obj, page_num):
             canvas_obj.saveState(); canvas_obj.setFont('NanumGothic', 7); company_info_line_height = 0.35 * cm; company_info_y = height - margin_y
        ...
        current_y = height - margin_y - 1*cm; draw_page_template(c, page_number); c.setFont('NanumGothicBold', 18); c.drawCentredString(width / 2.0, current_y, "이삿날 견적서(계약서)"); current_y -= line_height * 2
        styles = getSampleStyleSheet(); center_style = ParagraphStyle(name='CenterStyle', fontName='NanumGothic', fontSize=10, leading=14, alignment=TA_CENTER)
        service_text = """고객님의 이사를 안전하고 신속하게 책임지는 이삿날입니다."""; p_service = Paragraph(service_text, center_style); p_service_width, p_service_height = p_service.wrapOn(c, width - margin_x*2, 5*cm)
        ...
        # 비용 항목 처리 (날짜 할증 병합 및 기본 운임 비고 수정 적용됨)
        ...
        for i, item in enumerate(temp_items):
              if str(item[0]) == "기본 운임":
                 ...
                 if date_surcharge_index != -1 and date_surcharge_amount > 0:
                     try: item[1] = int(item[1]) + date_surcharge_amount; item[2] = "이사 집중일 추가 운영 요금"
        ...
        if date_surcharge_index != -1:
              ...
              except IndexError: print(f"Warning: Could not remove date surcharge item")
        ...
        if special_notes:
            ...
            for note_part in notes_parts:
                ...
                p_part.drawOn(c, margin_x, current_y - part_height); current_y -= (part_height + line_height * 0.2)
...
def generate_excel(state_data, calculated_cost_items, total_cost, personnel_info):
    ...
    try:
        ...
        # 2. '전체 품목 수량' 시트 데이터 생성 (utils.get_item_qty 호출)
        ...
        # 3. '비용 내역 및 요약' 시트 데이터 생성 (기존 로직 유지)
        cost_details_excel = [];
