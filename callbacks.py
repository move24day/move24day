...
def update_basket_quantities():
    ...
    then updates basket item quantities. Defaults to "1톤" if no vehicle is viable.
    This function is THE TRUTH for final_selected_vehicle and basket quantities.
    It should be called AFTER any action that might change the recommended or chosen vehicle.
    """
    ...
    if vehicle_choice_method == "자동 추천 차량 사용":
        ...
        # else: print(f"DEBUG CB: Auto - Recommended not valid or not in available trucks.")
        ...
        # else: print(f"DEBUG CB: Manual - Choice not valid or not in available trucks.")
    ...
    # Default to "1톤" if no vehicle is selected/recommended AND "1톤" is available and has defaults
    vehicle_key_for_defaults = vehicle_for_baskets
    if not vehicle_key_for_defaults: 
        if "1톤" in data.default_basket_quantities and "1톤" in available_trucks_for_type:
            vehicle_key_for_defaults = "1톤"
            # print(f"DEBUG CB: Baskets - No vehicle, defaulting to '1톤' for basket quantities.")
        # else: print(f"DEBUG CB: Baskets - No vehicle, and '1톤' defaults not found/available. Baskets will be 0.")
    
    # print(f"DEBUG CB: Baskets - vehicle_key_for_defaults='{vehicle_key_for_defaults}'")

    if vehicle_key_for_defaults and vehicle_key_for_defaults in data.default_basket_quantities:
        basket_defaults = data.default_basket_quantities[vehicle_key_for_defaults]
        # print(f"DEBUG CB: Baskets - Using defaults for '{vehicle_key_for_defaults}': {basket_defaults}")
        ...
        # print(f"DEBUG CB: Baskets - No valid vehicle for defaults ('{vehicle_key_for_defaults}') or no defaults defined. Setting all defined baskets to 0.")
        for item_name in defined_basket_items:
...
def handle_item_update():
    ...
    # This is crucial: after item updates change recommendations, refresh final vehicle and baskets.
...
def sync_move_type(widget_key):
    ...
    if not MOVE_TYPE_OPTIONS: st.error("이사 유형 옵션 누락."); return
    ...
    if widget_key in st.session_state:
        ...
        if st.session_state.get('base_move_type') != new_value:
            ...
            # When move type changes, item quantities for the *new* type might be all zero initially.
            # We must recalculate volume/weight (which handle_item_update does) and then update baskets.
