# ui_tab3.py
# "투입 인력 정보" 테이블 삭제 및 "이사 정보 요약" 추가 버전
...
def render_tab3():
    ...
    with st.container(border=True):
        ...
        # "실제 투입 차량" 섹션은 이전 요청에 따라 삭제된 상태로 가정 (또는 필요시 이 부분도 조정)
        ...
        if remove_opt:
            ...
            if "remove_base_housewife" in st.session_state: # 키가 존재하면 False로 설정 (체크박스가 안보일 때의 상태)
        ...
        with col_waste2:
            if st.session_state.get("has_waste_check"):
                ...
                # else: st.warning("data.WASTE_DISPOSAL_COST_PER_TON 정의 오류.") # 필요 시 주석 해제
    ...
    with st.container(border=True):
        ...
        num_cols_cost_adj = 3
        if st.session_state.get("has_via_point"): # 경유지가 있을 경우 컬럼 수 조정
            num_cols_cost_adj = 4
        ...
        if st.session_state.get("has_via_point"): # 경유지 있을 때만 표시
    ...
    total_cost = 0
    cost_items = []
    personnel_info = {} # personnel_info는 calculate_total_moving_cost에서 반환됨

    if final_selected_vehicle_calc:
        try:
            # 보관이사 기간 재계산 (만약 이사일/도착일 변경 시 필요)
            if st.session_state.get("is_storage_move"):
                ...
                if isinstance(moving_dt_recalc, date) and isinstance(arrival_dt_recalc, date) and arrival_dt_recalc >= moving_dt_recalc:
                    ...
                    st.session_state.storage_duration = max(1, delta_recalc.days + 1) # storage_duration 업데이트
                else: # 날짜가 유효하지 않으면 기본 1일
                    st.session_state.storage_duration = 1 

            current_state_dict = st.session_state.to_dict() # 현재 세션 상태를 가져옴
            if hasattr(calculations, "calculate_total_moving_cost") and callable(calculations.calculate_total_moving_cost):
                ...
                # PDF/이미지 생성을 위해 세션 상태에 저장 (다른 곳에서 이 값을 참조할 수 있음)
            ...
            deposit_amount_val = st.session_state.get("deposit_amount", 0) # 비용 조정에서 입력된 값 사용
            try: deposit_amount_num = int(deposit_amount_val) # 정수로 변환
            ...
            # 오류 문자열이 cost_items에 있는지 확인 (calculate_total_moving_cost에서 오류 시 문자열 반환 가능성)
            ...
            if error_item_found:
                ...
                elif not cost_items: # cost_items가 비어있는 경우
                    st.info("산출된 비용 내역이 없습니다.")
                else: # 형식이 맞지 않는 경우
                    st.error("비용 내역 데이터 형식이 올바르지 않습니다.")
            
            st.write("---") # "투입 인력 정보" 표시 전 구분선 (이전 버전에서의 위치 참고)
            ...
            summary_data = {
                ...
                "출발지 층/작업": f"{st.session_state.get('from_floor', '-')} / {st.session_state.get('from_method', '-')}",
                "도착지": st.session_state.get("to_location", "-"),
                "도착지 층/작업": f"{st.session_state.get('to_floor', '-')} / {st.session_state.get('to_method', '-')}",
            }

            if st.session_state.get("has_via_point", False):
                summary_data["경유지"] = st.session_state.get("via_point_location", "-")
            ...
            if st.session_state.get("is_storage_move", False):
                ...
                summary_data["(보관 후) 도착 예정일"] = str(arrival_date_summary) if arrival_date_summary else "-"


            if st.session_state.get("apply_long_distance", False):
                summary_data["장거리이사"] = st.session_state.get("long_distance_selector", "-")
    ...
    st.divider() # 다운로드 및 발송 버튼 섹션 이전 구분선
    ...
    with col_pdf_dl:
        if hasattr(pdf_generator, "generate_pdf") and callable(pdf_generator.generate_pdf):
            if not pdf_args["calculated_cost_items"] and pdf_args["total_cost"] == 0:
                ...
                pdf_data_cust_dl = pdf_generator.generate_pdf(**pdf_args) # 버튼 누르기 전에 생성
                if pdf_data_cust_dl:
                    st.download_button(
                        label="📄 PDF 견적서 다운로드",
    ...
    with col_excel_dl:
        if hasattr(excel_filler, "fill_final_excel_template") and callable(excel_filler.fill_final_excel_template):
            if not pdf_args["calculated_cost_items"] and pdf_args["total_cost"] == 0 :
                ...
                excel_bytes_dl = excel_filler.fill_final_excel_template( # 버튼 누르기 전에 생성
