# ui_tab3.py (Fixed NameError, UnboundLocalError, and added robustness to options block)
import streamlit as st
import pandas as pd
import io
import pytz
from datetime import datetime, date # NameError 해결
import traceback

# Import necessary custom modules
try:
    import data
    import utils # UnboundLocalError 해결 및 로직 안정성 위해 필요
    import calculations
    import pdf_generator
    import excel_filler
    import email_utils
    # import mms_utils # 필요시 주석 해제
    ...
    if 'utils' not in str(ie): st.warning("utils.py 파일이 필요합니다.")
...
def render_tab3():
    ...
    if move_type_options_local and isinstance(move_type_options_local, (list, tuple)):
        ...
        st.radio("기본 이사 유형:", options=move_type_options_local, index=current_index_tab3, horizontal=True,
        ...
        with col_v2_widget:
            ...
            if use_auto_widget:
                if final_vehicle_widget:
                    st.success(f"✅ 자동 선택됨: **{final_vehicle_widget}**"); spec = data.vehicle_specs.get(final_vehicle_widget) if hasattr(data, 'vehicle_specs') else None
                ...
                if not available_trucks_widget: st.error("❌ 현재 이사 유형에 선택 가능한 차량 정보가 없습니다.")
                    ...
                    if manual_selected_display:
                        st.info(f"ℹ️ 수동 선택됨: **{manual_selected_display}**"); spec_manual = data.vehicle_specs.get(manual_selected_display) if hasattr(data, 'vehicle_specs') else None
    ...
    # --- Work Conditions & Options (안정성 강화된 버전) ---
    with st.container(border=True):
        st.subheader("🛠️ 작업 조건 및 추가 옵션")

        # --- Inputs for additional personnel ---
        ...
        # --- Inputs for actual dispatched vehicles ---
        ...
        if final_vehicle_for_options and current_move_type_options:
            try:
                ...
                if final_vehicle_for_options in vehicle_prices_options_display:
                    base_info = vehicle_prices_options_display.get(final_vehicle_for_options, {}) # .get() 사용
                    base_w = base_info.get('housewife', 0) # housewife 키 없어도 오류 방지 (기본값 0)

                    # 기본 여성 인원이 있고 숫자인 경우에만 옵션 활성화
                    if isinstance(base_w, (int, float)) and base_w > 0:
                        remove_opt = True
                        # data.ADDITIONAL_PERSON_COST 안전하게 가져오기 (없으면 0)
                        additional_cost = getattr(data, 'ADDITIONAL_PERSON_COST', 0) # Default 0 if missing
                        # 추가 비용이 숫자인지 확인
                        if not isinstance(additional_cost, (int, float)):
                             st.warning("data.ADDITIONAL_PERSON_COST 가 숫자가 아닙니다. 할인 금액이 0으로 처리됩니다.")
                             additional_cost = 0
                        # 할인 금액 계산 (타입 오류 방지)
                        try:
                             discount_amount = additional_cost * base_w
                        except TypeError:
                             st.warning(f"할인 금액 계산 오류 (Types: {type(additional_cost)}, {type(base_w)}). 0으로 처리됩니다.")
                             discount_amount = 0
                # else: 선택된 차량에 대한 가격 정보 없음

            except Exception as e:
                # 예상치 못한 오류 발생 시 사용자에게 알림
                st.error(f"기본 여성 인원 옵션 처리 중 예상치 못한 오류: {e}")
                remove_opt = False # 오류 발생 시 옵션 비활성화

        # 체크박스 조건부 표시 (remove_opt가 True일 때만 표시)
        if remove_opt:
            st.checkbox(f"기본 여성({base_w}명) 제외 (비용 할인: -{discount_amount:,.0f}원)", key="remove_base_housewife")
        else:
            # 옵션이 비활성화되었을 때 해당 상태를 False로 확실하게 설정
        ...
        with col_waste2:
             # Use st.session_state.get for safe access
        ...
        # 키 생성 시 고유성 보장됨 (date_opt_0_widget 등)
    ...
    if final_selected_vehicle_calc:
        try:
            ...
            st.subheader("📋 이사 정보 요약"); summary_generated = False
            try:
                ...
                if excel_data_summary:
                    ...
                    if "견적 정보" in xls.sheet_names and "비용 내역 및 요약" in xls.sheet_names:
                        ...
                        def format_money_manwon_unit(amount):
                            try:
                                if amount is None or isinstance(amount, str) and not amount.replace(',','').replace('.','',1).isdigit(): return "0";
                            ...
                            except Exception as format_e: print(f"Error formatting amount '{amount}': {format_e}"); return "금액오류";
                        def get_cost_abbr_manwon_unit(kw, abbr, df):
                            if df.empty or '항목' not in df.columns or '금액' not in df.columns: return f"{abbr} 정보 없음";
                            for row in df.itertuples(index=False):
                                item_name = getattr(row, '항목', None); item_amount = getattr(row, '금액', None);
                                if item_name and isinstance(item_name, str) and item_name.strip().startswith(kw): formatted_amount = format_money_manwon_unit(item_amount); return f"{abbr} {formatted_amount}";
                            return f"{abbr} 정보 없음";
                        ...
                        from_addr = format_address(info_dict.get("출발지 주소", st.session_state.get('from_location',''))); to_addr = format_address(info_dict.get("도착지 주소", st.session_state.get('to_location',''))); phone = info_dict.get("고객 연락처", st.session_state.get('customer_phone','')); vehicle_type = final_selected_vehicle_calc; note = format_address(info_dict.get("고객요구사항", st.session_state.get('special_notes',''))); p_info = personnel_info if isinstance(personnel_info, dict) else {}; men = p_info.get('final_men', 0); women = p_info.get('final_women', 0); ppl = f"{men}+{women}" if women > 0 else f"{men}"; b_name = "포장 자재 📦"; move_t = st.session_state.base_move_type;
                        def get_qty(key_suffix):
                           try: return int(st.session_state.get(f"qty_{move_t}_{b_name}_{key_suffix}", 0))
                           except: return 0
                        q_b = get_qty("바구니"); q_m = get_qty("중박스") if f"qty_{move_t}_{b_name}_중박스" in st.session_state else get_qty("중자바구니"); q_c = get_qty("옷바구니"); q_k = get_qty("책바구니"); bask_parts = [];
                        if q_b > 0: bask_parts.append(f"바{q_b}"); if q_m > 0: bask_parts.append(f"중{q_m}"); if q_c > 0: bask_parts.append(f"옷{q_c}"); if q_k > 0: bask_parts.append(f"책{q_k}"); bask = " ".join(bask_parts); cont_fee_str = get_cost_abbr_manwon_unit("계약금 (-)", "계", df_cost); rem_fee_str = get_cost_abbr_manwon_unit("잔금 (VAT 별도)", "잔", df_cost); w_from = format_method(info_dict.get("출발 작업", st.session_state.get('from_method',''))); w_to = format_method(info_dict.get("도착 작업", st.session_state.get('to_method',''))); work = f"출{w_from}도{w_to}"; addr_separator = " - " if from_addr and to_addr else " "; first_line = f"{from_addr}{addr_separator}{to_addr} {vehicle_type}"; st.text(first_line.strip()); st.text("");
                        ...
                        st.text(work); st.text(""); st.text(f"{cont_fee_str} / {rem_fee_str}"); st.text("");
            ...
            if can_gen_final_excel:
                if st.button("📄 생성: Final 견적서", key="btn_gen_final_excel"):
                    try:
                        ...
                        if filled_excel_data: st.session_state['final_excel_data'] = filled_excel_data; st.success("✅ Final Excel 생성 완료!"); st.rerun()
                        else:
                ...
                if st.session_state.get('final_excel_data'):
                    # *** UnboundLocalError 해결됨: ph_part 정의 추가 ***
                    ...
                    fname = f"{ph_part}_{now_str}_Final견적서.xlsx" # 이제 ph_part 사용 가능
        ...
        # --- Column 2: PDF Actions ---
        with cols_actions[1]:
            st.markdown("**② 고객용 견적서 (PDF)**");
            if can_gen_pdf:
                if st.button("📄 PDF 생성/재생성", key="btn_gen_pdf"):
                    pdf_bytes = None;
                    try:
                        pdf_total_cost = st.session_state.get("final_adjusted_cost", 0);
                        if not cost_items or not personnel_info: pdf_total_cost, cost_items, personnel_info = calculations.calculate_total_moving_cost(st.session_state.to_dict())
                        if not isinstance(personnel_info, dict): personnel_info = {}
                        pdf_bytes = pdf_generator.generate_pdf(st.session_state.to_dict(), cost_items, pdf_total_cost, personnel_info); st.session_state['pdf_data_customer'] = pdf_bytes;
                        if pdf_bytes: st.success("✅ PDF 생성 완료!"); st.rerun();
                        else: st.error("❌ PDF 생성 실패.");
                        if not pdf_bytes and 'pdf_data_customer' in st.session_state: del st.session_state['pdf_data_customer']
                    except Exception as pdf_gen_err: st.error(f"PDF 생성 중 예외 발생: {pdf_gen_err}"); traceback.print_exc();
                    if 'pdf_data_customer' in st.session_state: del st.session_state['pdf_data_customer']
                if st.session_state.get('pdf_data_customer'):
                    pdf_bytes_dl = st.session_state.get('pdf_data_customer'); ph_part_dl = utils.extract_phone_number_part(st.session_state.get('customer_phone', ''), 4, "0000");
                    ...
                    fname_dl = f"{ph_part_dl}_{now_str_dl}_이삿날견적서.pdf"; st.download_button("📥 다운로드 (PDF)", pdf_bytes_dl, fname_dl, 'application/pdf', key='dl_pdf'); st.write("---"); st.markdown("**이메일 발송**"); default_email = st.session_state.get('customer_email', ''); recipient_email = st.text_input("받는 사람 이메일:", value=default_email, key="recipient_email_input");
                    if st.button("📧 PDF 이메일 발송"):
                        if recipient_email and "@" in recipient_email:
                            pdf_bytes_to_send = st.session_state.get('pdf_data_customer');
                            if pdf_bytes_to_send:
                                pdf_filename_for_email = fname_dl; email_subject = f"[이삿날] {st.session_state.get('customer_name','고객')}님 견적서입니다."; customer_name_email = st.session_state.get('customer_name', '고객'); email_body = f"""안녕하세요, {customer_name_email}님.\n요청하신 이삿날 견적서를 첨부하여 보내드립니다.\n\n감사합니다.""";
                                with st.spinner("📧 이메일 발송 중..."): send_success = email_utils.send_quote_email(recipient_email=recipient_email, subject=email_subject, body=email_body, pdf_bytes=pdf_bytes_to_send, pdf_filename=pdf_filename_for_email );
        ...
        with cols_actions[2]:
            ...
            try:
                # import mms_utils; import utils; # Ensure utils is imported earlier
                # Check if mms_utils was actually imported and functions exist
                if 'mms_utils' in globals() and callable(getattr(utils, 'convert_pdf_to_image', None)) and callable(getattr(mms_utils, 'send_mms_with_image', None)):
                    mms_available = True
            except NameError: # If mms_utils wasn't imported
                mms_available = False
            except Exception as import_err: # Catch other import errors
                 print(f"Error checking MMS availability: {import_err}")
                 mms_available = False

            if not mms_available: st.caption("MMS 발송 기능 비활성화됨 (필수 모듈/함수 없음)")
            elif can_gen_pdf:
                if st.session_state.get('pdf_data_customer'):
                    default_phone = st.session_state.get('customer_phone', ''); recipient_phone = st.text_input("받는 사람 휴대폰 번호:", value=default_phone, key="recipient_phone_input", placeholder="01012345678");
                    if st.button("📱 이미지 MMS 발송"):
                        if recipient_phone and len(recipient_phone) >= 10:
                            pdf_bytes_mms = st.session_state.get('pdf_data_customer');
                            if pdf_bytes_mms:
                                image_bytes = None;
                                with st.spinner("🔄 PDF를 이미지로 변환 중..."):
                                    try:
                                        if callable(getattr(utils, 'convert_pdf_to_image', None)): image_bytes = utils.convert_pdf_to_image(pdf_bytes_mms, fmt='JPEG')
                                        else: st.error("❌ PDF 이미지 변환 함수(utils.convert_pdf_to_image) 없음.")
                                    except Exception as img_conv_err: st.error(f"❌ 이미지 변환 오류: {img_conv_err}"); traceback.print_exc()
                                if image_bytes:
                                    ...
                                    image_filename = f"{ph_part_mms}_{now_str_mms}_견적서.jpg"; customer_name_mms = st.session_state.get('customer_name','고객'); mms_text = f"{customer_name_mms}님, 요청하신 이삿날 견적 이미지를 보내드립니다.";
                                    with st.spinner(f"📱 MMS 발송 중... ({recipient_phone})"):
                                         try:
                                             if 'mms_utils' in globals() and callable(getattr(mms_utils, 'send_mms_with_image', None)): mms_success = mms_utils.send_mms_with_image(recipient_phone=recipient_phone, image_bytes=image_bytes, filename=image_filename, text_message=mms_text)
                                             else: st.error("❌ MMS 발송 함수(mms_utils.send_mms_with_image) 없음."); mms_success = False
                                         except Exception as mms_err: st.error(f"❌ MMS 발송 중 오류: {mms_err}"); traceback.print_exc(); mms_success = False;
                                    if mms_success: st.success(f"✅ '{recipient_phone}' 번호로 MMS 발송 성공.")
                                else: pass # Error message shown during conversion if needed
