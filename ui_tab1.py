# ui_tab1.py (Email input added, Load debug removed, Save uses actual filename)
...
try:
    ...
    # Import necessary state manager components
    ...
    # Import callbacks - ensure this module exists and contains the functions
    import callbacks
...
def render_tab1():
    ...
    with st.container(border=True):
        ...
        with col_load:
            ...
            if st.button("🔍 견적 검색"):
                ...
                if search_term_strip:
                    ...
                    if results:
                        ...
                        # Find the name corresponding to the first ID
            ...
            if st.session_state.get('gdrive_search_results'):
                 ...
                 # Try to find the index of the currently selected filename
                 if st.session_state.gdrive_selected_filename in file_options_display:
                     try:
                         ...
                         current_selection_index = 0 # Default to first if index not found

                 # Get the callback function safely
                 on_change_callback_gdrive = getattr(callbacks, 'update_selected_gdrive_id', None)
                 st.selectbox(
                     "불러올 JSON 파일 선택:",
                     options=file_options_display,
                     key="gdrive_selected_filename_widget", # Unique key for widget state
                     index=current_selection_index,
                     on_change=on_change_callback_gdrive # Pass callback reference
                 )
                 # Initial sync if needed (e.g., after search button click)
            ...
            if st.button("📂 선택 견적 불러오기", disabled=load_button_disabled, key="load_gdrive_btn"):
                ...
                if json_file_id:
                    ...
                    if loaded_content:
                        # Get basket update callback safely
                        ...
                        if not update_basket_callback_ref:
                             ...
                             # Provide a dummy function to prevent crash if callback is missing
                        ...
                        if load_success:
                            ...
                            if image_filenames_to_load:
                                st.session_state.loaded_images = {} # Clear previously displayed images
                                ...
                                for i, img_filename in enumerate(image_filenames_to_load):
                                     ...
                                     with st.spinner(f"이미지 '{img_filename}' 검색 중..."):
                                         # Use find_file_id_by_exact_name (assuming it handles uniqueness if needed)
                                     ...
                                     if img_file_id:
                                         ...
                                         # DEBUG message removed
                                     # Keep delay for API rate limits if needed
                                     time.sleep(0.1)
                                img_load_bar.empty() # Remove progress bar after loop
                                ...
                                # DEBUG message removed
                            # DEBUG message removed (for no images found case)
                        # Error message displayed within load_state_from_data if load fails
                    # DEBUG message removed (for JSON load failure)
                    # DEBUG message removed
        ...
        with col_save:
            ...
            with st.form(key="save_quote_form"):
                ...
                st.caption(f"사진 파일명 형식: `{quote_base_name}_사진1.png` 등 (중복 시 이름 뒤에 번호 추가됨)") # Updated caption
                ...
                st.caption("JSON(견적 데이터) 파일은 덮어쓰기됩니다. 사진은 매번 새로 업로드됩니다 (중복 시 자동 이름 변경).") # Updated caption
                ...
                if submitted:
                    ...
                    if phone_part == "번호없음" or not customer_phone.strip():
                        ...
                        # --- Image Upload and Filename Saving (Modified) ---
                        saved_image_names = [] # List to store actual saved filenames
                        ...
                        for i, uploaded_file in enumerate(files_to_upload):
                            ...
                            # Generate the DESIRED filename (may be changed by Drive helper)
                            ...
                            with st.spinner(f"이미지 '{desired_drive_image_filename}' 업로드 시도 중..."):
                                ...
                                # Call the modified save_image_file (handles unique name)
                                save_img_result = gdrive.save_image_file(desired_drive_image_filename, image_bytes)

                            # Check result and use ACTUAL saved name
                            if save_img_result and save_img_result.get('id'):
                                 # Use the filename returned by Google Drive helper
                                 actual_saved_name = save_img_result.get('name', desired_drive_image_filename)
                                 saved_image_names.append(actual_saved_name) # Append the actual name
                        ...
                        # Update session state with ACTUAL saved filenames BEFORE preparing JSON
                        st.session_state.gdrive_image_files = saved_image_names
                        # Prepare data (including the updated gdrive_image_files list) for saving
    ...
    st.header("📝 고객 기본 정보")
    # Get move type options and callback safely
    ...
    if move_type_options_tab1:
        ...
        # Display move type radio button
        st.radio( "🏢 **기본 이사 유형**",
                  options=move_type_options_tab1,
                  index=current_index_tab1,
                  horizontal=True,
                  key="base_move_type_widget_tab1",
                  on_change=sync_move_type_callback_ref, # Pass callback reference
                  args=("base_move_type_widget_tab1",) # Args for callback
                )
    ...
    with col1:
        ...
        # Ensure METHOD_OPTIONS exists before accessing
    ...
    with col2:
        ...
        # Ensure moving_date is a date object on init or load
        if not isinstance(current_moving_date_val, date):
             ...
             st.session_state.moving_date = default_date_def # Set default if not date
        # Display date input
        st.date_input("🗓️ 이사 예정일 (출발일)", key="moving_date")
        # Display quote creation time
