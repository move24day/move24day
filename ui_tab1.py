...
def render_tab1():
            ...
            if st.button("📂 선택 견적 불러오기", disabled=load_button_disabled, key="load_gdrive_btn"):
                ...
                if json_file_id:
                    ...
                    if loaded_content:
                        ...
                        if not update_basket_callback_ref: st.error("Basket update callback not found!"); update_basket_callback_ref = lambda: None
                        ...
                        if load_success:
                            ...
                            if image_filenames_to_load:
                                num_images = len(image_filenames_to_load); img_load_bar = st.progress(0, text=f"🖼️ 이미지 로딩 중... (0/{num_images})"); loaded_count = 0
                                for i, img_filename in enumerate(image_filenames_to_load):
                                     ...
                                     if img_file_id:
                                         ...
                                         if img_bytes: st.session_state.loaded_images[img_filename] = img_bytes; loaded_count += 1; progress_val = (i + 1) / num_images; img_load_bar.progress(progress_val, text=f"🖼️ 이미지 로딩 중... ({loaded_count}/{num_images})")
                    ...
                    else: st.error("❌ 선택된 JSON 파일 내용을 불러오는 데 실패했습니다.")
                st.rerun()
    ...
    st.header("📝 고객 기본 정보"); move_type_options_tab1 = globals().get('MOVE_TYPE_OPTIONS'); sync_move_type_callback_ref = getattr(callbacks, 'sync_move_type', None)
