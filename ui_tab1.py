# ui_tab1.py (Restored Customer Info, Uploader outside form, WITH key, NO state reset, NO explicit rerun in submit)
...
def render_tab1():
    ...
    with st.container(border=True):
        ...
        with col_save:
            ...
            # Keep the uploader OUTSIDE the form, WITH key
            ...
            try:
                ...
                # Handle the specific recurring error gracefully
                ...
                # Handle any other unexpected error during rendering
                st.error(f"파일 업로더 렌더링 중 예상치 못한 오류: {general_ex}")
                traceback.print_exc()
            # --- End try-except block ---

            # Only render the form if the uploader rendered successfully
            if uploader_rendered_successfully:
                with st.form(key="save_quote_form"):
                    ...
                    if submitted:
                        ...
                        # Add extra check for key existence for safety
                        if "quote_image_uploader" not in st.session_state:
                             st.warning("파일 업로더 상태를 찾을 수 없습니다. 페이지를 새로고침 해보세요.")
                             current_uploaded_files = []
                        else:
                             current_uploaded_files = st.session_state.quote_image_uploader or []
                        ...
                        if phone_part == "번호없음" or not customer_phone.strip():
                            ...
                            # REMOVED explicit rerun
                # --- End Form ---
            # else: If uploader failed to render, form is not shown
