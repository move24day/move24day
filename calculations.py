# calculations.py (get_floor_num 함수 수정됨, 디버깅 추가됨)
        ...
        for section, item_list in item_defs.items():
            ...
            if isinstance(item_list, list):
                for item_name in item_list:
                    ...
                    widget_key = f"qty_{move_type}_{section}_{item_name}"; qty_raw = state_data.get(widget_key); qty = 0
                    ...
                    if qty > 0:
                        ...
                        except KeyError: print(f"Error: Item '{item_name}' not found.")
    ...
    for truck_name, specs in sorted_trucks:
        ...
        if usable_capacity > 0 and usable_weight >= 0:
            if total_volume <= usable_capacity and total_weight <= usable_weight:
                recommended_vehicle = truck_name; remaining_space_percent = (1 - (total_volume / usable_capacity)) * 100 if usable_capacity > 0 else 0; break
...
def calculate_total_moving_cost(state_data):
    ...
    print("-" * 30) # DEBUG: 구분선
    print(f"DEBUG: Starting cost calculation for vehicle: {selected_vehicle}") # DEBUG
    ...
    print(f"DEBUG: Base cost calculated: {total_cost}") # DEBUG
    ...
    sky_total_cost = 0; sky_notes_list = []
    sky_hours_from = state_data.get('sky_hours_from', 1); sky_hours_final = state_data.get('sky_hours_final', 1)
    ...
    if from_method == "스카이 🏗️":
        ...
        if sky_hours_from > 1: note += f" + 추가 {data.SKY_EXTRA_HOUR_PRICE*(sky_hours_from-1):,}"
        sky_total_cost += cost; sky_notes_list.append(note)
    if to_method == "스카이 🏗️":
        cost = data.SKY_BASE_PRICE + data.SKY_EXTRA_HOUR_PRICE * (sky_hours_final - 1)
        note = f"도착({sky_hours_final}h): {data.SKY_BASE_PRICE:,}"
        if sky_hours_final > 1: note += f" + 추가 {data.SKY_EXTRA_HOUR_PRICE*(sky_hours_final-1):,}"
        sky_total_cost += cost; sky_notes_list.append(note)
    if sky_total_cost > 0: cost_items.append(("스카이 장비", sky_total_cost, " | ".join(sky_notes_list))); total_cost += sky_total_cost

    add_men = state_data.get('add_men', 0); add_women = state_data.get('add_women', 0)
    ...
    try: adjustment_amount = int(adjustment_amount_raw)
    ...
    if is_storage:
        duration = state_data.get('storage_duration', 1)
    ...
    try: tons = float(tons) if tons is not None else 0.5; tons = max(0.5, tons)
    except: tons = 0.5
    if has_waste: cost = data.WASTE_DISPOSAL_COST_PER_TON * tons; cost_items.append(("폐기물 처리(톤)", cost, f"{tons:.1f}톤 기준")); total_cost += cost

    date_surcharge = 0; date_notes = []
